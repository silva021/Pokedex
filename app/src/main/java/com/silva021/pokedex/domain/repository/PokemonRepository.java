...
public class PokemonRepository {
    ...
    public LiveData<List<Pokemon>> getAllPokemonByGeneration(int page, int generation) {
        final MutableLiveData<List<Pokemon>> data = new MutableLiveData<>();
        ServiceGenerator.createService(MyPokemonApiService.class).getAllPokemonByGeneration(generation, page).enqueue(new Callback<List<Pokemon>>() {
            @Override
            public void onResponse(Call<List<Pokemon>> call, Response<List<Pokemon>> response) {
                data.setValue(response.body());
            }
